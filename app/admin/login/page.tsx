...
export default function AdminLoginPage() {
  ...
  // Fonction commune pour vérifier si l'utilisateur est admin dans Firestore
  const verifyAndRedirectAdmin = async (user: any) => {
    ...
    if (userDoc.exists() && userDoc.data().isAdmin === true) {
      ...
      setError("Accès refusé : ce compte ne possède pas les droits administrateur.");
      await auth.signOut();
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await verifyAndRedirectAdmin(userCredential.user);
    } catch (err: any) {
  ...
  // 2. Connexion rapide avec Google
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      await verifyAndRedirectAdmin(result.user);
    } catch (err: any) {
      ...
      <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-8 shadow-sm space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex items-center gap-2">
            ...
            <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-mono font-semibold">ADMIN SECURE</span>
          </div>
          <p className="text-xs text-slate-500">Connectez-vous pour accéder au panneau d'administration.</p>
        </div>
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-4 text-slate-400 text-[10px] uppercase">Ou par email</span>
          <div className="flex-grow border-t border-slate-100"></div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="admin@nourstore.com"
          ...
          >
            {loading ? "Vérification en cours..." : "Se connecter à l'Admin"}
          </button>
        </form>
